...
canvas.grid(row=1, column=0,rowspan=140, padx=20)
...
def get_news():
    ...
    for n in range(len(stories)):
        ...
        vars()[dynamic_var] = \
            canvas.create_text(0, i, text=f"{stories[n]['story']}",
                               font=("Arial", 16),
                               fill="#ff6600", anchor=NW, tags="textTag")
        i += 27
        x+=1
        if links.get() == 1:
            dynamic_var = f"label{x}"
            vars()[dynamic_var] = \
                canvas.create_text(0, i, text=f"{stories[n]['link']}",
                                   font=("Arial", 16),
                                   fill="#56a5f5", activefill="yellow", anchor=NW, tags="textTag")

            canvas.tag_bind(vars()[dynamic_var],"<Button-1>", lambda e, link=stories[n]['link']:
            open_link(link))
            i += 27
            x+=1
        if summaries.get() == 1:
            dynamic_var = f"label{x}"
            vars()[dynamic_var] = \
                canvas.create_text(0, i, text=f"{stories[n]['summary']}",
                                   font=("Arial", 16),
                                   fill="#f55882", anchor=NW, tags="textTag")
            i += 27
            x+=1
    ...
    story_results.config(text=f"{len(stories)} Stories Displayed")
...
links_button.grid(row=5, column=4, columnspan=8)
summaries_button.grid(row=6, column=4, columnspan=8)
...
get_stories.grid(row=40, column=4,columnspan=7)
story_results.grid(row=50,column=3,columnspan=9)
